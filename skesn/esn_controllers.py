...
class HomotopyController(Controller):
    ...
    def __init__(self, use_transfer_learning = True, anchor_signal = 0.5, mu=1e-2): 
    ...
    def prereg(self, hiddens, targets, sig): 
        ...
        signals = list(set(sig[:, 0, 0]))
        idx = int(len(signals) * self.anc_sig_)

        # First calculate the anchor matrix for transfer
        signals = sorted(signals)
        signals[0], signals[idx] = signals[idx], signals[0]
        ...
        for i in range(len(signals)):
            ...
            if(i == 0): main_ids = ids.copy()
            if(i == 0 or not self.use_tl_):
                if self.esn.regularization == 'l2':
